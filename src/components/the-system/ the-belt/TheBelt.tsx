...
class TheBelt extends React.Component<Props, unknown, React.Context<SystemContext>> {
  ...
  memoizedRenderBelt(beltProperties: BeltProperties, scrollToRef: RefObject<HTMLDivElement> | null): ReactNode {
  ...
  render(): ReactNode {
    ...
    return (
      <div>
        <div className={styles.beltLayer} style={this.getBeltStyle(innerBelt, outerBelt, true)}>
          {this.memoizedRenderBelt(beltProperties, scrollToRef)}
        </div>
        <div className={styles.beltLayer} style={this.getBeltStyle(innerBelt, outerBelt, false)}>
